...
%               osses2022
%
%   Url: http://amtoolbox.org/amt-1.1.0/doc/experiments/exp_osses2022.php
...
Colours = {'b',il_rgb('Green'),'r',il_rgb('LightSkyBlue'),il_rgb('Maroon'),'m','k',il_rgb('mediumorchid')}; 
Markers = {'o','s','<','>','v','^','p','d'};
...
if flags.do_fig4
    ...
    for idx_lvl = idxs2look 
        for k = 1:K
            ...
            P = il_InterX(L1,L2);
            fprintf('%s; freq=%.0f; amp=%.1f dB re max\n',models{k},fm1ERB,P(2));
    ...
    for ii = 1:length(fc2look)
        for k = 1:K
            ...
            for idx_lvl = 1:3
                ...
                % [xx, xx,xx,fc_here] = il_get_freq(models{k},lvls(idx_lvl),fc2look);
                ...
                pp = audtofreq(P(1,:));
                fprintf('%s; fc = %.1f Hz; BW=%.1f Hz\n',models{k},(pp(2)+pp(1))/2,pp(2)-pp(1));
                ...
                if flags.do_plot
                    ...
                    ylabel(Ylabel);
                    % ylabel('     Output','HorizontalAlignment','Right');
                    if idx_lvl == 1
                        ha2(end+1) = gca;
                    end
                end
                ...
                switch models{k}
                    case 'dau1997'
                        for j = 1:N
                            ...
                            out_freq_resp_all(:,end+1) = il_rmsdb(out);
                            fc_resp_ref = fc_resp;
                        end
                    ...
                    case 'verhulst2015'
                        ...
                        for j = 1:N
                            ...
                            out_freq_resp_all(:,end+1) = il_rmsdb(out_model(j).v);
                        end
                ...
                case 1 % Low intensity
                  ...
                  for ii = 1:length(fc_ref)
                    ...
                    [BW_low_each(ii,:,k), QERB_low_each(ii,:,k), ...
                     ...
                     = il_Get_ERB_estimation_multi(outsig(:,ii), fc_ref(ii), fs, N_sections);
...
if flags.do_fig13
    ...
    for k = 1:length(models)
        ...
        if flags.do_plot
            ...
            plot(tf_i_here+env_std_here, ampl_here,'o--','Color',il_rgb('Maroon'),'LineWidth',2,'MarkerFaceColor','w');
            plot(tf_i_here*[1 1],YL,'k-');

            tf_f_here = tf_i_here + 2*3;
            y_here = 6.8;
            plot([tf_i_here tf_f_here],y_here*[1 1],'-','Color',il_rgb('Maroon'),'LineWidth',2);

            amp_val = 0;
            for ii=1:4
                tf_f_here = tf_i_here + amp_val;
                plot(tf_f_here*[1 1],[y_here-.2 y_here],'-','Color',il_rgb('Maroon'),'LineWidth',2);
