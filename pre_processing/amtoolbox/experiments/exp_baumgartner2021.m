...
%
%   Url: http://amtoolbox.org/amt-1.1.0/doc/experiments/exp_baumgartner2021.php
...
elseif flags.do_fig4
  ...
  for ics = 1:length(cueSelLbl)
    ...
    [Pext{ics},Stmp,Wtmp,MSEtmp,BICtmp,RCtmp] = mapAndDecide(...
...
%        'BIC_WSM','BIC_LTA','BIC_MTA','BIC_WTA',...
  ...
  %% Performance (RMSE) as a function of # considered cues
  y = {RMSE,RC};
  ci = {RMSE_CI,RC_CI};
...
elseif flags.do_fig2
  exp_baumgartner2021('all')

elseif flags.do_tabSingleCue || flags.do_fig3
...
function [Pext,S,W,MSE,BIC,RC] = mapAndDecide(predCue,actual,Erange,Eoffset,S,Wfix,expnum)
...
S.LTA=fminsearch(@(s) f_predErr(f_select(predCueMtx,s,'LTA')),S.WSM,o);
S.MTA=fminsearch(@(s) f_predErr(f_select(predCueMtx,s,'MTA')),S.WSM,o);
S.WTA=fminsearch(@(s) f_predErr(f_select(predCueMtx,s,'WTA')),S.WSM,o);

% Externalization predictions
predExtComb(:,:,1) = f_weight(predExtSingle,W); % Oracle
predExtComb(:,:,2) = f_weight(predExtSingle,Wfix); % WSM
[predExtComb(:,:,3),cueStat(:,1)] = f_select(predCueMtx,S.LTA,'LTA');
[predExtComb(:,:,4),cueStat(:,2)] = f_select(predCueMtx,S.MTA,'MTA');
[predExtComb(:,:,5),cueStat(:,3)] = f_select(predCueMtx,S.WTA,'WTA');
...
function RC = f_cumrankcorr(p,actual,expnum)
...
if exist('expnum','var')
  ...
  for ii = 1:Ncues+Nstrat
    ...
    RC.m(ii) = f_cumrankcorr(p,actual,expnum);
    RC.ci(ii,:) = bootci(300,@f_cumrankcorr,p,actual,expnum);
  end
end
