...
%
%   Url: http://amtoolbox.org/amt-1.1.0/doc/core/amt_emuexp.php
    ...
    case 'modelinit'
        ...
        for outputcounter = 1:outputnumber
            if any(kvmodel.outputs==outputcounter)
                modelstring = ['modelout.par',outputcounter];
            ...
            switch par.exp.interface
              case 'AMT'
                ...
                for count = 1:modeloutvarcount-1
                    modeloutname = ['par', par.model.outputs(count)];
                    amd_disp(modeloutname);
