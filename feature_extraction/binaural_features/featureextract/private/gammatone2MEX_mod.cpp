// Gammatone filtering ...
// Note that this implementaion is based on the old code from Tobias
...
#define   IN_STATE               prhs[8]  // state from prev block

/* Output Arguments */
#define   BM     				 plhs[0]  // Basilar membrane displacement
#define   ENV     				 plhs[1]  // Envelope
#define   OUT_STATE              plhs[2]  // state for next block
...
void usage()
{
	...
	mexPrintf("\n");
	mexPrintf("\n USAGE: ");
	mexPrintf("\n\t[BM,STATE] = gammatone(IN,FS,LOWFREQ,UPFREQ,NFILTER,BEAR,BALIGN,BINFO,STATE);");
	mexPrintf("\n");
	mexPrintf("\n INPUT PARAMETER:  ");
...
{
	double *input, *bm, *env, *filter, *pqptr;
	...
	int    bTimeAlign = false, bHaveState = false;
	...
	if(!bExit){
		...
		for (chan = 0; chan < nFilter2Process; chan++)
		{
            ...
            if (bHaveState) {
                //get state from the input
                if (infoFlag) mexPrintf( "Init from input\n" );
                for (ii = 0; ii < 4; ii++) {
                    fChan[chanIdx].p[ii] = *pqptr++;
                    fChan[chanIdx].q[ii] = *pqptr++;
                }
            } else {
                // Initialize gammatone filter states to 0
                if (infoFlag) mexPrintf( "Init 0\n" );
                for (ii = 0; ii < 4; ii++)
                {
            ...
            for (nn = 0; nn < nSamples; nn++)
            {
                ...
                // *==========================================
                bm[nn+chan*nSamples] = fChan[chanIdx].p[3] * gain;

                // *==========================================
                // * Envelope
                // *==========================================
                if (nlhs>1){
                    env[nn+chan*nSamples] = sqrt(fChan[chanIdx].p[3] 
                            * fChan[chanIdx].p[3] + fChan[chanIdx].q[3] 
                            * fChan[chanIdx].q[3]) * gain;
                }
        ...
        // Copy p and q back into state if desired
        if (nlhs>1){
            OUT_STATE = mxCreateDoubleMatrix(8, nFilter2Process, mxREAL);
            pqptr = mxGetPr(OUT_STATE);
            for (chan = 0; chan < nFilter2Process; chan++)
            {
                chanIdx = processFilter[chan];
                for (ii = 0; ii < 4; ii++) {
                    *pqptr++ = fChan[chanIdx].p[ii];
                    *pqptr++ = fChan[chanIdx].q[ii];
                }
            }
        }
