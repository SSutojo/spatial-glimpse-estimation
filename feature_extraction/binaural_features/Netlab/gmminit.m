function mix = gmminit(mix, x, options, kMethod)
...
% Select default initialization method
if nargin < 4 || isempty(kMethod); kMethod = 'kmeans'; end
...
% Chose cluster initialization
switch kMethod
    case 'kmeans'
        % =================================================================
        % Kmeans of the NETLAB package
        % =================================================================
        %
        % Use feature space to initialize centers
        options(5) = true;
        % Run kMeans
        [mix.centres, options, post] = kmeans(mix.centres, x, options); 
    case 'kmeanshar'
        % =================================================================
        % Harmonic Kmeans clustering
        % =================================================================
        %                
        expon = 4;
        [mix.centres,perf,tmpPost] = kmeanhar(x,mix.ncentres,options(14)...
                                              +options(3),expon,...
                                              mix.centres);
        % Map 'post'
        post = zeros(mix.ncentres,ndata);
        % Warp indices to work on a matrix
        postIdx = tmpPost + (0:ndata-1)' * mix.ncentres;
        % Transform indices
        post(postIdx) = 1;
        % Transpose
        post = transpose(post);
    case 'kmeansvb'
        % =================================================================
        % Kmeans of the VOICEBOX toolbox
        % =================================================================
        %
        [mix.centres,perf,tmpPost] = kmeansvb(x,mix.ncentres,'f',...
                                              options(14)+options(3));
        % Map 'post'
        post = zeros(mix.ncentres,ndata);
        % Warp indices to work on a matrix
        postIdx = tmpPost + (0:ndata-1)' * mix.ncentres;
