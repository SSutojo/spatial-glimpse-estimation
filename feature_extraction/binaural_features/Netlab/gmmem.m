...
if options(5) > 0
  ...
  MIN_COVAR    = options(5); % Minimum singular value of covariance matrix
...
%  new_pr = sum(post, 1);% auskommentiert Sarinah 10.4.18
  new_pr = sum(post, 1)+ eps;% einkommentiert Sarinah 10.4.18, ein Fehler wird zu den priors addiert, um Nullen zu vermeiden (besonders bei overfitting)
  new_c = post' * x;
...
%           mix.covars(j,:)/max(mix.covars(j,:)) * MIN_COVAR; 
