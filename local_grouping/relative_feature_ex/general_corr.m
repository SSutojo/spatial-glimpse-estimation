function gamma = general_corr(vec1, vec2)
%CALCULATE THE GENERAL CORRELATION COEFFICIENT BETWEEN TWO DATASETS OF SAME SIZE
% vectors must be of same size
% INPUTS:   vec1 - vector with dataset 1
%           vec2 - dataset 2 (same length as vec1)
% OUTPUTS:  gamma - correlation coefficient between vec1 and vec2
...
elseif mean(vec1==vec2)~=1 && sum(vec1-repmat(mean(vec1),length(vec1),1))~=0 && sum(vec2-repmat(mean(vec2),length(vec2),1))~=0
    ...
    R_numerator = sum(vec1.*vec2);
    R_denominator = sqrt(sum(vec1.^2)*sum(vec2.^2));
    gamma = R_numerator/R_denominator;    
else
