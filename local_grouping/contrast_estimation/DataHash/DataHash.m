...
%   DataHash(int32(1:10), Opt)  % m0On/vTKhJHgbLWa3A+Cdw
% % SHA-1, Base64:
%   S.a = uint8([]);
%   S.b = {{1:10}, struct('q', uint64(415))};
%   Opt.Method = 'SHA-1';
%   Opt.Format = 'HEX';
%   DataHash(S, Opt)            % 69DF1D743641A419B22F08684234F687FC2B6698
...
% $JRev: R-C V:028 Sum:K01Rv83nrBji Date:15-Oct-2015 23:12:19 $
...
elseif usetypecastx  % Array with type, Faster typecastx:
   Engine = CoreHash_(Data, Engine);
   Hash   = typecastx(Engine.digest, 'uint8');
   
else                 % Array with type, Slower built-in TYPECAST:
...
elseif isnumeric(Data) || islogical(Data) || ischar(Data)
   if isempty(Data) == 0
      if isreal(Data)                % TRUE for LOGICAL and CHAR also:
         Engine.update(typecastx(Data(:), 'uint8'));
      else                           % typecastx would accept complex input:
         Engine.update(typecastx(real(Data(:)), 'uint8'));
         Engine.update(typecastx(imag(Data(:)), 'uint8'));
      end
   end
...
elseif (isobject(Data) || isjava(Data)) && ismethod(Data, 'hashCode')
   Engine = CoreHash_(char(Data.hashCode), Engine);
   ...
   try
      Engine = CoreHash_(BasicData, Engine);
   catch ME
